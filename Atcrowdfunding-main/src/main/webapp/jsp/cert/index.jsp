        ...
        <script type="text/javascript">
            			...
            			if ( result.success ) {
            				...
            				$.each(certList, function(i, cert){
             				   content = content +  '<tr>';
          					   ...
          					   content = content +  '      <button type="button" onclick="window.location.href=\'${pageContext.request.contextPath}/cert/edit.htm?pageno='+pageObj.pageNum+'&id='+cert.id+'\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
            ...
            function deleteCert(id, name) {
    			layer.confirm("删除账号为"+name+"资质, 是否继续？",  {icon: 3, title:'提示'}, function(cindex){
    				// 删除数据
    				$.ajax({
    					url : "${pageContext.request.contextPath}/cert/delete.do",
    					type : "POST",
    					data  : {
    						id : id
    					},
            ...
            function deleteCerts() {
            	...
            	if ( checkBox.length == 0 ) {
        			...
        			layer.confirm("删除选择的资质数据, 是否继续？",  {icon: 3, title:'提示'}, function(cindex){
        				...
        				$.ajax({
        					url : "${pageContext.request.contextPath}/cert/deletes.do",
        					type : "POST",
        					data  : obj,
        					success : function(result) {
        						if ( result.success ) {
